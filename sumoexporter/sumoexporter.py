...
from yaramo.model import DbrefGeoNode, EuclideanGeoNode, Wgs84GeoNode
from yaramo.node import EdgeConnectionDirection
from yaramo.signal import SignalDirection

from .model import Point, Route, Signal, Track
...
class SUMOExporter(object):
    ...
    def convert_topology(self):
        ...
        for yaramo_node in self.topology.nodes.values():
            point_obj = Point(
                yaramo_node.uuid, yaramo_node.geo_node.uuid, yaramo_node.name
            )
            point_obj.x, point_obj.y = _get_shifted_coords_of_yaramo_geo_node(
                yaramo_node.geo_node
            )
        ...
        for yaramo_edge in self.topology.edges.values():
            ...
            missing_nodes = yaramo_edge.intermediate_geo_nodes + [
                yaramo_node_b.geo_node
            ]

            for inter_yaramo_geo_node in missing_nodes:
                edge_length = previous_yaramo_geo_node.get_distance_to_other_geo_node(
                    inter_yaramo_geo_node
                )
                for cur_signal in signal_list:
                    if (
                        cur_signal.id not in processed_signals
                        and cur_signal.distance_from_start
                        < edge_distance_sum_so_far + edge_length
                    ):
                        ...
                        (
                            signal_x,
                            signal_y,
                        ) = yaramo_edge.get_coordinates_on_edge_by_distance_from_start_node(
                            float(cur_signal.distance_from_start)
                        )
                        cur_signal.x, cur_signal.y = _get_shifted_coords_by_type(
                            signal_x, signal_y, type(inter_yaramo_geo_node)
                        )

                        cur_signal.left_track = cur_track_obj
                        cur_track_obj.right_point = cur_signal
                        cur_track_obj.add_shape_coordinates(
                            f"{cur_signal.x},{cur_signal.y}"
                        )

                        # Create new track
                        cur_track_top_kanten_counter = cur_track_top_kanten_counter + 1
                        cur_track_obj = Track(
                            yaramo_edge.uuid, cur_track_top_kanten_counter
                        )
                        cur_track_obj.add_shape_coordinates(
                            f"{cur_signal.x},{cur_signal.y}"
                        )
    ...
    def convert_routes(self):
        def _get_track_ids_in_order(_start_signal, _end_signal, _yaramo_edges_in_order):
            ...
            if _cur_dir == SignalDirection.IN:
                ...
                if self.add_edge_before_first_signal:
                    _track_ids.append(
                        _start_signal.left_track.id
                    )  # To start train before signal
                _track_ids.append(_cur_track.id)
            else:  # "gegen" direction
                _cur_track = _start_signal.left_track
                if self.add_edge_before_first_signal:
                    _track_ids.append(
                        _start_signal.right_track.re_id
                    )  # To start train before signal
                ...
                if _next_point.is_point():
                    ...
                    for _yaramo_edge in _yaramo_edges_in_order:
                        if _yaramo_edge.uuid != _cur_top_kante_uuid:
                            _next_track = _next_point.get_connected_track(
                                _yaramo_edge.uuid
                            )
                    ...
                    if _next_track is None:
                        raise ValueError(
                            f"Data structure broken, no following TOP Kante on route from {_start_signal.id} to {_end_signal.id} found"
                        )
        ...
        for yaramo_route in self.topology.routes.values():
            ...
            if (
                yaramo_route.start_signal is None
                or yaramo_route.start_signal.uuid not in self.signals
                or yaramo_route.end_signal is None
                or yaramo_route.end_signal.uuid not in self.signals
            ):
                print(
                    f"Skip route because at least one signal not found, route uuid: {yaramo_route.uuid}"
                )
                continue
            ...
            # Tracks
            route.track_ids = _get_track_ids_in_order(
                route.start_signal, route.end_signal, yaramo_route.get_edges_in_order()
            )
